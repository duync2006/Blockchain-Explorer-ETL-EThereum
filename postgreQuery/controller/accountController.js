...
const AccountController = {
  getAsset: async (req, res) => {
    try {
      ...
      const address_to_check_sum = await web3.utils.toChecksumAddress(
        req.params.address
      );
      const ETH_balancePromise = web3.eth.getBalance(address_to_check_sum);
      const total_transactionPromise =
        web3.eth.getTransactionCount(address_to_check_sum);
      const bytecodePromise = web3.eth.getCode(address);
      const [ETH_balance, total_transaction, bytecode] = await Promise.all([
        ETH_balancePromise,
        total_transactionPromise,
        bytecodePromise,
      ]);
      let result = {};
      ...
      if ((result.type = "EOA")) {
        ...
        const [last_txn_sent, first_txn_sent] = await Promise.all([
          first_txn_sent_Promise,
          first_txn_sent_Promise,
        ]);
      ...
      const uniqueTokenAddressSet = new Set(
        asset.map((item) => item.token_address)
      );
      ...
      const tokenPromises = paginateArr.map(async (token) => {
        ...
        try {
          balance = await contract.methods
            .balanceOf(address_to_check_sum)
            .call();
        } catch (error) {}
  ...
  getAccountERC20Overview: async (req, res) => {
    try {
      ...
      const address = req.params.address.toLowerCase();
      const ERC20Contract = await prisma.contracts.findUnique({
        where: { address: address },
      });
      if (!ERC20Contract.is_erc20)
  ...
  getAccountERC721Overview: async (req, res) => {
    try {
      ...
      const address = req.params.address.toLowerCase();
      const addressToCheckSum = await web3.utils.toChecksumAddress(address);
      const ERC721Contract = await prisma.contracts.findUnique({
        where: { address: address },
      });
      if (!ERC721Contract.is_erc721)
        return res.status(500).json({
          success: false,
      ...
      const resultObject = holders.reduce((accumulator, currentValue) => {
        ...
        if (!accumulator[holderKey]) {
          accumulator[holderKey] = {
            holder: holderKey,
            tokenIDs: [currentValue.tokenIDs],
          };
        } else {
      ...
      finalResultArray = finalResultArray.sort(
  ...
  getERC721_item: async (req, res) => {
    try {
      ...
      (result.NTF_item = NFT), (result.tokenId = id);
